...
export async function generateMetadata({ params }) {
  ...
  if (!podcast) {
    return {
      title: 'Podcast Not Found | Sport Endorse',
      description: 'The requested podcast episode could not be found.'
    }
  }
  
  const title = decodeHtmlEntities(podcast.title?.rendered || 'Podcast Episode')
  const description = podcast.yoast_head_json?.description || 
    (podcast.excerpt?.rendered ? podcast.excerpt.rendered.replace(/<[^>]*>/g, '').substring(0, 160) : 'Listen to this podcast episode on Sport Endorse.')
  
  return {
    title: `${title} | Sport Endorse`,
    description: decodeHtmlEntities(description),
    alternates: {
      canonical: `https://www.sportendorse.com/podcasts/${resolvedParams.slug}`,
      languages: {
