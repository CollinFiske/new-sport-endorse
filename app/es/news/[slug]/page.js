...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.sportendorse.com/es/news/${resolvedParams.slug}`
    },
    openGraph: {
      title: newsStory.title.rendered,
          ...
          <article className="blog-post-article">
            <header className="blog-post-article-header">
              ...
              <div className="blog-post-article-meta">
                <time>{new Date(newsStory.date).toLocaleDateString('es-ES')}</time>
                {newsStory._embedded?.author?.[0] && (
                  <span>Por {newsStory._embedded.author[0].name}</span>
                )}
              </div>
